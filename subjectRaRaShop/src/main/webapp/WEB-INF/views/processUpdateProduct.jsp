...
//update 시 setInt() 메소드를 위해 int형으로 cating
...
//Database 테이블로 update
	...
	try{
		...
		if(rs.next()){
			...
			if(fileName!=null){//1) 요청 파라미터 중에서 이미지 파일이 있으면 실행
				sql = "UPDATE PRODUCT "
					+ "	SET    P_NAME=?,P_UNITPRICE=?,P_DESCRIPTION=?,P_CATEGORY=?,"
				...
				sql = "UPDATE PRODUCT "
					+ "	SET    P_NAME=?,P_UNITPRICE=?,P_DESCRIPTION=?,P_CATEGORY=?,"
					+ " P_MANUFACTURER=?,P_UNITSINSTOCK=?,P_CONDITION=?"
					+ " WHERE  P_ID=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1,name);        
				pstmt.setString(2,unitPrice);   
